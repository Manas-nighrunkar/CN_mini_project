...
class HTTPServer():
	...
	def sendCookie(self):
		if(self.clientAddress not in self.cookielist):
			...
			print(self.cookielist, self.clientAddress)
			return s
	...
	def errorHeaders(self, responseLine, error):
		...
		if os.path.exists(filename):
			...
			responseBody = "<h1>{} Not Found</h1>".format(error)
	...
	def GETMethod(self, request):
		...
		if(os.path.exists(filename)):
			...
			if(not r):
				...
				headers = self.errorHeaders(responseLine, 403)
				statusCode = 403
				self.client.send((responseLine + headers).encode())
				return
		try:
		...
		print(filename)
		if os.path.exists(filename):
			responseLine = self.responseLine(request, 200)
			statusCode = 200
	...
	def HEADMethod(self, request):
		...
		if os.path.exists(filename):
			...
			responseLine = self.responseLine(request, 404)
			headers = self.errorHeaders(responseLine, 404)

	def POSTMethod(self, request):
		blank_line = "\r\n"
		extra_headers = {}
		special_characters = {'%21': '!', '%23': '#', '%24': '$', '%25': '%', '%26': '&', '%27': "'", '%28': '(', '%29': ')', '%2A': '*',
        ...
        #extra_headers["Content-Location"] = filename OR
        #extra_headers["Location"] = filename
		try:
	...
	def receiveRequest(self, client, clientAddress):
		...
		i = 0
		while True:
			p = self.client.recv(8000) #Considering sum of the request line and ALL header fields is <= 8000
			data += p
			...
			elif(flag == 1 and len(data) < 8000): #for head and get
				break
...
class TCPServer():
	...
	def createSocket(self):
		...
		while True:
			...
			#print("New request received from: ", client)
